...
<body>
    ...
    <form action="boardsList.do" method="get">
        ...
        <select name="type">
            ...
            <option value="writer" <%= "writer".equals(request.getParameter("type")) ? "selected" : "" %>>작성자</option>
        </select>
        
        <!-- 정렬 기준 선택 -->
        <select name="sort">
            <option value="title" <%= "title".equals(request.getParameter("sort")) ? "selected" : "" %>>제목</option>
            <option value="createTime" <%= "createTime".equals(request.getParameter("sort")) ? "selected" : "" %>>작성일</option>
            <option value="userNumber" <%= "userNumber".equals(request.getParameter("sort")) ? "selected" : "" %>>작성자</option>
        </select>

        <!-- 오름차순/내림차순 선택 -->
        <select name="order">
            <option value="asc" <%= "asc".equals(request.getParameter("order")) ? "selected" : "" %>>오름차순</option>
            <option value="desc" <%= "desc".equals(request.getParameter("order")) ? "selected" : "" %>>내림차순</option>
        </select>
