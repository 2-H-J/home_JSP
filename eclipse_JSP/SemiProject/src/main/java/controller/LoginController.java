...
/**
 * LoginController는 클라이언트의 로그인 요청을 처리하는 컨트롤러입니다. 
 * 클라이언트로부터 아이디와 비밀번호를 입력받아 인증을 수행하고,
 * 성공 또는 실패 여부에 따라 적절한 뷰로 이동합니다.
 */
public class LoginController implements Controller {
    ...
    public ModelAndView execute(HttpServletRequest request, HttpServletResponse response)
        ...
        if (dto != null) {
            ...
            System.out.println("[LoginController] 리다이렉트 설정 -> Path: ./index.jsp");
        } else {
            // 4. 로그인 실패 처리
            view.setPath("./loginView.jsp?error=invalid"); // 실패 메시지를 URL 파라미터로 전달
            view.setRedirect(false); // forward 방식으로 전달
