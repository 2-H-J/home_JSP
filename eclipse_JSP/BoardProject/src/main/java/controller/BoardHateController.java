...
// BoardHateController 클래스는 게시글에 "싫어요" 기능을 처리하는 컨트롤러입니다.
// 클라이언트 요청으로부터 게시글 번호와 사용자 정보를 받아 "싫어요"를 추가하거나 취소한 후,
// 작업 결과를 JSON 형태로 응답합니다.
public class BoardHateController implements Controller {

    // execute 메서드는 클라이언트 요청을 처리하고 결과를 반환하는 메서드입니다.
    // - HttpServletRequest: 클라이언트의 요청 정보를 담고 있습니다.
    // - HttpServletResponse: 서버에서 클라이언트로 보낼 응답 정보를 담고 있습니다.
    // - ModelAndView: 요청 처리 결과와 뷰(View)로 이동하기 위한 경로를 포함하는 객체입니다.
    @Override
    public ModelAndView execute(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        // [INFO] 게시글 싫어요 요청 처리 시작
        System.out.println("[INFO] 게시글 싫어요 요청 처리 시작");

        // 1. 게시글 번호(bno) 추출
        // - 요청 파라미터에서 게시글의 고유 번호를 가져옵니다.
        // - 게시글 번호는 "싫어요" 작업을 수행할 게시글을 식별하는 데 사용됩니다.
        ...
        // - 사용자 ID는 "싫어요" 작업이 수행된 사용자를 식별하는 데 사용됩니다.
        ...
        try {
            // 4. 싫어요 추가 처리
            // - BoardService의 insertBoardHate 메서드를 호출하여 싫어요를 추가합니다.
            // - 싫어요가 정상적으로 추가되면 성공 메시지를 JSON에 추가합니다.
            System.out.println("[INFO] 싫어요 추가 처리 시작");
            BoardService.getInstance().insertBoardHate(bno, id); // 싫어요 추가 로직 실행
            json.put("msg", "해당 게시글에 싫어요를 하셨습니다."); // 성공 메시지 저장
            System.out.println("[INFO] 싫어요 추가 처리 완료");
        } catch (Exception e) {
            // 5. 싫어요 취소 처리
            // - 예외 발생 시 이미 추가된 싫어요를 취소합니다.
            // - BoardService의 deleteBoardHate 메서드를 호출하여 기존 싫어요를 삭제합니다.
            System.out.println("[ERROR] 싫어요 처리 중 예외 발생: " + e.getMessage());
            System.out.println("[INFO] 싫어요 취소 처리 시작");
            BoardService.getInstance().deleteBoardHate(bno, id); // 싫어요 취소 로직 실행
            json.put("msg", "해당 게시글에 싫어요를 취소하셨습니다."); // 취소 메시지 저장
            System.out.println("[INFO] 싫어요 취소 처리 완료");
        }
        ...
        System.out.println("[INFO] 게시글 싫어요 요청 처리 완료");
        return null;
    }
}
