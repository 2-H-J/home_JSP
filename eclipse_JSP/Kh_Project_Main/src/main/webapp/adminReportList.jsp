    ...
    <div class="body-container">
        ...
        <div class="pagination" style="margin-top: 20px; text-align: center;">
            <c:if test="${currentPage > 1}">
                <a href="${pageContext.request.contextPath}/adminReports.do?page=${currentPage - 1}&memberId=${param.memberId}">
                    <button style="padding: 8px 12px; border-radius: 4px; background-color: #007bff; color: white; border: none; cursor: pointer;">이전</button>
                </a>
            </c:if>
            <c:if test="${currentPage == 1}">
                <button style="padding: 8px 12px; border-radius: 4px; background-color: #b0b0b0; color: white; border: none; cursor: not-allowed;">이전</button>
            </c:if>

            <c:forEach var="i" begin="1" end="${totalPages}">
                <a href="${pageContext.request.contextPath}/adminReports.do?page=${i}&memberId=${param.memberId}">
                    <button style="padding: 8px 12px; border-radius: 4px; background-color: ${i == currentPage ? '#0056b3' : '#007bff'}; color: white; border: none; cursor: pointer; margin: 0 5px;">
                        ${i}
                    </button>
                </a>
            </c:forEach>
            ...
            <c:if test="${currentPage < totalPages}">
                <a href="${pageContext.request.contextPath}/adminReports.do?page=${currentPage + 1}&memberId=${param.memberId}">
                    <button style="padding: 8px 12px; border-radius: 4px; background-color: #007bff; color: white; border: none; cursor: pointer;">다음</button>
                </a>
            </c:if>
            <c:if test="${currentPage == totalPages}">
                <button style="padding: 8px 12px; border-radius: 4px; background-color: #b0b0b0; color: white; border: none; cursor: not-allowed;">다음</button>
            </c:if>
        </div>
